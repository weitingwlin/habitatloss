function [t, X] = LVmetaGillespie4(X0, tlim, dist, k , sB, EA, EB, cA, cB, bAB, bBA)
if nargin<10
...
while t(point) < tlim    
    ...
    % 3. witch event
     ...
     % 4. update state                                       
         ...
         % setination site
         if event == 5
             dest_patch = datasample( 1:P, 1, 'weight', dispA(patch, :) ); % chose a destination patch
             X(dest_patch, : , point + 1) = X(dest_patch, :, point+1) - v{event};  % add immigrant
         end
         if event == 6
             dest_patch = datasample( 1:P, 1, 'weight', dispB(patch, :) ); % chose a destination patch
             X(dest_patch, : , point + 1) = X(dest_patch, :, point+1) - v{event};  % add immigrant
         end
